...
export function ResumeCard({
  ...
  const getStatusColor = (status: Resume['status']) => {
    switch (status) {
      case 'draft':
        return 'bg-status-caution-bg text-status-caution border-status-caution-bg';
      case 'completed':
  ...
  return (
    <motion.div
      className="bg-white border border-line-400 rounded-xl p-6 hover:shadow-md hover:border-primary-200 transition-all duration-200 group"
    ...
    >
        ...
        <button
          ...
          className={cn(
            ...
            resume.isPublic
              ...
              : 'text-label-400 hover:bg-label-100'
          )}
      ...
      {statistics && (
        <div className="grid grid-cols-3 gap-4 py-3 mb-4 border-t border-line-200">
          <div className="text-center">
            <div className="text-caption-1 font-semibold text-label-900">
              {statistics.totalViews.toLocaleString()}
            </div>
            <div className="text-caption-3 text-label-500">지원 건수</div>
          </div>
          <div className="text-center">
            <div className="text-caption-1 font-semibold text-label-900">
              {statistics.downloadCount.toLocaleString()}
            </div>
            <div className="text-caption-3 text-label-500">열람 횟수</div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-line-200">
        <div className="flex items-center gap-2">
          <button
            onClick={onView}
            className="flex items-center gap-2 px-3 py-1.5 text-caption-2 font-medium text-primary-600 hover:bg-primary-50 rounded-lg transition-colors cursor-pointer"
          >
            <Eye size={14} />
            미리보기
          </button>
          ...
          {statistics && (
            <button className="flex items-center gap-2 px-3 py-1.5 text-caption-2 font-medium text-label-600 hover:bg-label-100 rounded-lg transition-colors cursor-pointer">
              <Download size={14} />
              다운로드
            </button>
          )}
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            onClick={onDelete}
            className="p-1.5 text-label-500 hover:text-status-error hover:bg-status-error-bg rounded-lg transition-colors cursor-pointer"
            title="삭제"
          >
            <Trash2 size={14} />
          </button>
