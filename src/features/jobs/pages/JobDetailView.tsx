    ...
    <Layout>
      ...
      <Modal isOpen={showApplyModal} onClose={() => setShowApplyModal(false)} title={t('selectResume')} size="sm">
        <div className="space-y-4">
          <p className="text-body-3 text-label-600">{t('selectResumeDesc')}</p>
          ...
          {isLoadingResumes ? (
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-lg border-2 transition-all cursor-pointer text-left ${
                    selectedResumeId === resume.id
                      ...
                      : 'border-line-400 hover:border-primary-200 hover:bg-label-50'
                  }`}
                >
                  <div className={`p-2 rounded-lg shrink-0 ${selectedResumeId === resume.id ? 'bg-primary-100' : 'bg-label-100'}`}>
                    <FileText size={18} className={selectedResumeId === resume.id ? 'text-primary-600' : 'text-label-500'} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-body-3 font-semibold text-label-900 truncate">{resume.title}</p>
                    <p className="text-caption-2 text-label-400 mt-0.5">
            ...
            <div className="py-8 text-center">
              <FileText size={40} className="mx-auto text-label-300 mb-3" />
              <p className="text-body-3 font-medium text-label-700 mb-1">{t('noResume')}</p>
              <p className="text-caption-2 text-label-400 mb-4">{t('noResumeDesc')}</p>
              <Link
                href="/user/resume/create"
                onClick={() => setShowApplyModal(false)}
                className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-primary-600 text-white text-body-3 font-semibold rounded-lg hover:bg-primary-700 transition-colors cursor-pointer"
              >
                {t('createResume')}
              </Link>
            </div>
          ...
          {resumes.length > 0 && (
            <div className="flex gap-3 pt-3 border-t border-line-200">
              <button
                onClick={() => setShowApplyModal(false)}
                className="flex-1 py-2.5 border border-line-400 text-label-600 text-body-3 font-semibold rounded-lg hover:bg-label-50 transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-primary-600 text-white text-body-3 font-semibold rounded-lg hover:bg-primary-700 disabled:bg-label-300 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
      ...
      <div className="min-h-screen bg-white py-4 sm:py-6 pb-24 lg:pb-8">
        <div className="page-container">
          ...
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-label-600 hover:text-primary-600 text-body-3 font-medium mb-4 sm:mb-6 cursor-pointer transition-colors"
          >
            <ChevronLeft size={18} />
            {t('backToListFull')}
          </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 lg:gap-8">
            ...
            <div className="space-y-6">
              {/* Hero Header Card */}
              <div className="bg-white rounded-xl border border-line-400 overflow-hidden">
                <div className="p-6 lg:p-8 space-y-6">
                  ...
                  <div className="flex items-start gap-4 justify-between">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-caption-2 font-semibold text-label-400 mb-2">{t('companyPost')}</p>
                    ...
                    <div className="hidden lg:flex items-center gap-2 shrink-0">
                      <motion.button
                        ...
                        className="p-2 rounded-lg text-label-400 hover:text-primary-600 hover:bg-primary-50 transition-colors cursor-pointer focus:outline-none"
                        whileTap={{ scale: 0.9 }}
              ...
              {/* Details Card */}
              <div className="bg-white rounded-xl border border-line-400 p-5 sm:p-7 lg:p-8 space-y-6">
                <h2 className="text-title-5 sm:text-title-4 font-extrabold text-label-900">{t('jobDetails')}</h2>

                {/* Job Content Section */}
                <div className="space-y-3">
                  <h3 className="text-body-3 font-bold text-label-900 flex items-center gap-2">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {language.map((lang, index) => (
                          <span
                            key={index}
                            className="inline-flex items-center px-2.5 py-1 bg-primary-50 text-primary-700 text-caption-2 font-semibold rounded-full border border-primary-200"
                          >
                            {lang}
                          </span>
                        ))}
            ...
            <div className="hidden lg:block">
              <motion.div
                className="bg-white rounded-xl border border-line-400 p-6 sticky top-[calc(65px+20px)] space-y-4"
                initial={{ y: 0 }}
                whileInView={{ y: 0 }}
              >
                {/* Salary Display */}
                ...
                {/* Apply Button */}
                {isExpired ? (
                  <button className="w-full py-4 bg-label-100 text-label-400 text-center rounded-lg font-bold text-body-2 cursor-not-allowed">
                    {t('expiredPostShort')}
                  </button>
                ) : (
                  <motion.button
